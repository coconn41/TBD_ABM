lay_eggs = function(tick_agents){
  ...
  tick_agents <<- tick_agents %>%
    mutate(Lifestage = ifelse(Lifestage=="Adult" & sex == "female" & mated == 1 & tick_age_wks == ,# find tick egg laying timing
                              "Eggs",Lifestage))
  
  new_eggs = tick_agents %>%
    filter(Lifestage=="Eggs") %>%
    mutate(Infection_status = ifelse(Lifestage=="Eggs","None",Infection_status),
         num_ticks = ifelse(Lifestage == "Eggs",1000,1),#Double check the 1000 eggs estimate
         ...
         sex = ifelse(Lifestage=="Eggs",sample(x = sexes, size = .)),

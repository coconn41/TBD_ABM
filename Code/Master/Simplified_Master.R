...
rm(list=setdiff(ls(), c("deer_density_sweep",
                        ...
                        "sweepcount",
                        "net_select")))
if(file.exists(paste0(getwd(),'/Parameter_sweeps/Round_2/tickdata_net_',
                      ...
                      '_overpop.csv'))==T |
   file.exists(paste0(getwd(),'/Parameter_sweeps/Round_2/tickdata_net_',
                      net_select,'_dd_',deer_density,
                      '_md_',mouse_density,
                      '_td_',tick_density,
                      '_mcc_',mouse_carrying_capacity,
...
if(rfdb==F){
  ...
  #####
  # Agents and patches:
  source(paste0(getwd(),'/Code/Model_set_up/Simple_load.R'))
...
#   select(-c(Ha_infected,Ha_infection_timer))}
  ...
  if(pathogens == F){tick_data2 <- tick_data2 %>%
    select(-c(ha_perc,v1_perc,tot_v1,tot_ha))}
  ...
  if(pathogens == F){deer_data2 <- deer_data2 %>%
    select(-c(tot_v1_infected,v1_perc))}
...
if(pathogens == T){mouse_agents = mouse_agents %>%
    mutate(Ha_infected = case_when(layer%in%starting_layers & Ha_infected == 0 ~ rbinom(n=n(),size=1,prob=.39),
           ...
           tick_links = 0)}else{mouse_agents$tick_links = 0}
...
if(rfdb==T){
  ...
  tick_data3 = data.frame(Lifestage = character(tdflength),
                          ...
                          simulation_day = numeric(tdflength),
                          simulation_week = numeric(tdflength))
  if(pathogens == F){tick_data3 <- tick_data3 %>%
    select(-c(ha_perc,v1_perc,tot_v1,tot_ha))}
  deer_data3 = data.frame(network_ID = numeric(hdflength),
                          ...
                          network = numeric(hdflength),
                          Agent = character(hdflength))
  if(pathogens == F){deer_data3 <- deer_data3 %>%
    select(-c(tot_v1_infected,v1_perc))}
  mouse_data3 = data.frame(network_ID = numeric(hdflength),
                           ...
                           network = numeric(hdflength),
                           Agent = character(hdflength))
  if(pathogens == F){mouse_data3 <- mouse_data3 %>%
    select(-c(tot_ha_infected,ha_perc))}
...
for(i in start_time:go_timesteps){ 
  ...
  if(day>=lay_egg){
    ...
    if(min(lengths(deer_agents$tick_links)[which(deer_agents$jump_patch==1)])<deer_carrying_capacity&
    ...
    available_mouse_agents = mouse_agents %>% # Updated
      filter(lengths(tick_links)<mouse_carrying_capacity) # Updated
    ...
    tick_agents <- T_matches5 %>%
      mutate(selection = case_when(deer_links>0&mouse_links>0&Lifestage=="Nymph" ~ rbinom(n=n(),size = 1,prob = 1),# N_prob),
                                   deer_links>0&mouse_links>0&Lifestage=="Larvae" ~ rbinom(n=n(),size=1,prob = 1),#L_prob),
  ...
  if(day>=lay_egg){ 
    ...
    if(pathogens==T){tick_agents <- tick_agents %>%
      ...
      mutate(tick_age_wks = ifelse(Lifestage=="Eggs" & tick_age_wks == -1,
                                   0,tick_age_wks))}
    if(pathogens==F){tick_agents <- tick_agents %>%
      mutate(num_ticks = ifelse(Lifestage=="Eggs" & tick_age_wks == -1,
                                eggs_per_female,num_ticks),#Double check the 1000 eggs estimate
             time_since_fed = ifelse(Lifestage=="Eggs" & tick_age_wks == -1,
                                     0,time_since_fed),
             dropped = ifelse(Lifestage=="Eggs" & tick_age_wks == -1,
  ...
  if(pathogens==T){mouse_agents <- mouse_agents %>%
    mutate(Age = Age+1,
           Ha_infection_timer = case_when(Ha_infected == 1 & Ha_infection_timer==0 ~ 1,
                                          ...
                                          TRUE ~ 0))}else{mouse_agents = mouse_agents %>%
                                            mutate(Age = Age+1)}#,
  ...
  if(pathogens==T){deer_agents <- deer_agents %>%
    mutate(Age = ifelse(Kill==1,0,Age),
           ...
           Kill = 0,
           susceptible = 1)}#ifelse(Kill==1,rbinom(n=n(),size=1,prob=.289),susceptible))
  if(pathogens==F){deer_agents <- deer_agents %>%
    mutate(Age = ifelse(Kill==1,0,Age),
           tick_links = ifelse(Kill==1,NA,tick_links),
           Kill = 0)}
  ...
  if(pathogens==T){mouse_agents <- mouse_agents %>%
    mutate(Age = ifelse(Kill==1,0,Age),
           ...
           # V1_infected = ifelse(Kill==1,0,V1_infected),
           Kill = 0,
           susceptible = 1)}#ifelse(Kill==1,rbinom(n=n(),size=1,prob=.39),susceptible))
  if(pathogens==F){mouse_agents <- mouse_agents %>%
    mutate(Age = ifelse(Kill==1,0,Age),
           tick_links = ifelse(Kill==1,NA,tick_links),
           Kill = 0)}
  ...
  # Compile results
  #####
  if(pathogens==T){deer_data <- deer_agents %>%
  ...
  tick_data <- tick_agents %>%
    ...
    summarise(ha_perc = (length(which(Infection_status == "ha"))/n())*100,
              ...
              network = net_select,
              season = season)}
  if(pathogens==F){deer_data <- deer_agents %>%
    group_by(network_ID,layer) %>%
    summarise(tot_deer = n()) %>%
    mutate(day_of_year = day,
           season = season,
           timestep = i,
           year = year,
  ...
  tick_data <- tick_agents %>%
    group_by(Lifestage,network_ID,layer) %>%
    summarise(total_ticks = sum(num_ticks),
              Agent = "Tick",
              year = year,
              timestep = i,
              day_of_year = day,
...
write.csv(tick_data2,
          paste0(getwd(),'/Parameter_sweeps/Round_2/tickdata_net_',net_select,
                 '_dd_',deer_density,
                 '_md_',mouse_density,
                 '_td_',tick_density,
                 '_mcc_',mouse_carrying_capacity,
